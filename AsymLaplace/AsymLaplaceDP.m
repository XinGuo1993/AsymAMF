...
temp0    = (U*V').*Y;
E_inv_s = ones(K,1);   % 对E（s2）初始化，因为一开始在计算sigma的分布的时候s的估计还是未知,或许随机初始化？
E_log_s = ones(K,1);   % 对E（s2）初始化，因为一开始在计算sigma的分布的时候s的估计还是未知,或许随机初始化？
...
%          end
        ...
        beta_s =  1+(0.5-tao_s)^2/(tao_s*(1-tao_s)) * phi(k); 
        gamma_s = 0.5 * phi(k) - 1;
        sq_ab = sqrt(alpha_s*beta_s);
        E_s(k) = sqrt(beta_s/alpha_s)*besselk(gamma_s+1,sq_ab)/besselk(gamma_s,sq_ab);
        E_inv_s(k) = sqrt(alpha_s/beta_s)*besselk(gamma_s+1,sq_ab)/besselk(gamma_s,sq_ab) - 2*gamma_s/beta_s ;
